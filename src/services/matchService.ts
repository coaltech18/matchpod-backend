 ...
 */
const TIMELINE_COMPATIBILITY: Record<string, string[]> = {
 ...
 * Includes display-only fields for human decision making
 */
export interface CandidateProfile {
  ...
  timeline: string;
  // Soft signals - display only, not used for matching
  lifestyle?: {
    smoking?: boolean;
    pets?: boolean;
    nightOwl?: boolean;
 ...
 */
function timelinesCompatible(userTimeline: string, candidateTimeline: string): boolean {
 ...
 */
function getOppositeRole(role: string): string {
 ...
 */
function createCandidateProfile(user: IUser): CandidateProfile {
  return {
 ...
 */
async function getAlreadySwipedUserIds(userId: string): Promise<string[]> {
 ...
 */
async function safeRedisSet(key: string, data: any, ttl: number): Promise<void> {
  try {
    const redis = getRedisClient();
    if (!redis.isOpen) {
      return;
 ...
 */
async function safeRedisScanDelete(pattern: string): Promise<number> {
  try {
    const redis = getRedisClient();
    if (!redis.isOpen) {
      return 0;
 ...
 */
export async function findMatches(userId: string): Promise<CandidateProfile[]> {
  ...
  const user = await UserModel.findById(validUserId).lean() as IUser | null;
  if (!user) {
  ...
  const query: any = {
    _id: {
      $ne: new Types.ObjectId(validUserId),
      $nin: alreadySwipedIds.map(id => new Types.ObjectId(id))
    },
    isActive: true,
    role: targetRole,
    city: user.city,
 ...
 */
export async function invalidateMatchesCacheForUser(userId: string): Promise<number> {
  const validUserId = validateUserId(userId);
