...
const profileUpdateSchema = z.object({
  ...
  role: z.enum(['has_room', 'seeking_room']).optional(),
  age: z.number().min(18, 'Must be 18 or older').max(100, 'Must be 100 or younger').optional(),
  ...
  ).max(6, 'Maximum 6 user photos').optional(),
  roomPhotos: z.array(
    z.string().refine(
      (val) => val.startsWith('demo:') || /^https?:\/\/.+/.test(val),
      { message: 'Invalid room photo reference' }
    )
  ).max(6, 'Maximum 6 room photos').optional(),
  budget: z.object({
    min: z.number().min(0, 'Budget min must be positive'),
    max: z.number().min(0, 'Budget max must be positive'),
  }).optional(),
  timeline: z.enum(['immediately', 'soon', 'flexible']).optional(),
  lifestyle: z.object({
    smoking: z.boolean(),
    pets: z.boolean(),
    nightOwl: z.boolean(),
 ...
 */
function validateRequiredFields(userData: any): { field: string; message: string }[] {
    ...
    try {
      ...
      Object.keys(updatePayload).forEach(key => {
        if (updatePayload[key as keyof typeof updatePayload] !== undefined) {
          sanitizedUpdate[key] = updatePayload[key as keyof typeof updatePayload];
        }
      });
