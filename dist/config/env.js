...
exports.isBetaOtpMode = isBetaOtpMode;
exports.getJwtConfig = getJwtConfig;
exports.getRedisConfig = getRedisConfig;
exports.getEmailConfig = getEmailConfig;
exports.getMatchWeights = getMatchWeights;
...
const envSchema = zod_1.z.object({
    ...
    MONGODB_URI: zod_1.z
        ...
        .refine((uri) => uri.startsWith('mongodb://') || uri.startsWith('mongodb+srv://'), 'MONGODB_URI must be a valid MongoDB connection string'),
    // ---------------------------------------------------------------------------
    ...
    JWT_ISSUER: zod_1.z.string().default('matchpod-api'),
    JWT_AUDIENCE: zod_1.z.string().default('matchpod-app'),
    // ---------------------------------------------------------------------------
    // Security
    // ---------------------------------------------------------------------------
    CORS_ORIGIN: zod_1.z.string().default('*'),
    ENCRYPTION_SECRET: zod_1.z.string().optional(),
    // ---------------------------------------------------------------------------
    // Redis (Optional - app works without it)
    // ---------------------------------------------------------------------------
    ENABLE_REDIS: zod_1.z
        .string()
    ...
    EMAIL_ENABLED: zod_1.z
        ...
        .transform((val) => val === 'true')
        .default('false'),
    EMAIL_PROVIDER: zod_1.z.enum(['log', 'sendgrid', 'smtp']).default('log'),
    EMAIL_FROM: zod_1.z.string().email().default('noreply@matchpod.app'),
    EMAIL_FROM_NAME: zod_1.z.string().default('MatchPod'),
    SENDGRID_API_KEY: zod_1.z.string().optional(),
    APP_URL: zod_1.z.string().url().default('https://matchpod.app'),
    // ---------------------------------------------------------------------------
    // Feature Flags (Beta Configuration)
    // ---------------------------------------------------------------------------
    BETA_STATIC_OTP: zod_1.z
        .string()
        .transform((val) => val === 'true')
        .default('true'), // Enabled by default for beta
    ENABLE_PUSH_NOTIFICATIONS: zod_1.z
        .string()
        .transform((val) => val === 'true')
        .default('false'),
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    API_RATE_LIMIT: zod_1.z.coerce.number().int().positive().default(100),
    API_RATE_WINDOW_MS: zod_1.z.coerce.number().int().positive().default(60000),
    AUTH_RATE_LIMIT: zod_1.z.coerce.number().int().positive().default(5),
    AUTH_RATE_WINDOW_MS: zod_1.z.coerce.number().int().positive().default(900000),
    OTP_RATE_LIMIT: zod_1.z.coerce.number().int().positive().default(3),
    OTP_RATE_WINDOW_MS: zod_1.z.coerce.number().int().positive().default(600000),
    // ---------------------------------------------------------------------------
    // Match Algorithm Weights (must sum to 1.0)
    // ---------------------------------------------------------------------------
    MATCH_WEIGHT_BUDGET: zod_1.z.coerce.number().min(0).max(1).default(0.25),
    MATCH_WEIGHT_LOCATION: zod_1.z.coerce.number().min(0).max(1).default(0.20),
    MATCH_WEIGHT_LIFESTYLE: zod_1.z.coerce.number().min(0).max(1).default(0.20),
    MATCH_WEIGHT_SCHEDULE: zod_1.z.coerce.number().min(0).max(1).default(0.15),
    MATCH_WEIGHT_CLEANLINESS: zod_1.z.coerce.number().min(0).max(1).default(0.10),
    MATCH_WEIGHT_PETS: zod_1.z.coerce.number().min(0).max(1).default(0.05),
    MATCH_WEIGHT_GENDER: zod_1.z.coerce.number().min(0).max(1).default(0.05),
    // ---------------------------------------------------------------------------
    // Logging (Optional)
    // ---------------------------------------------------------------------------
...
function validateEnv() {
    ...
    const totalWeight = env.MATCH_WEIGHT_BUDGET +
...
function getConfig() {
    if (!validationComplete || !config) {
        throw new Error('Configuration not initialized. Call validateEnv() at application startup before accessing config.');
    }
    return config;
}
/**
