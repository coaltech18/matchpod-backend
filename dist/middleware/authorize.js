...
exports.requireMessagePermission = requireMessagePermission;
exports.requireSwipePermission = requireSwipePermission;
exports.requireProfileUpdate = requireProfileUpdate;
exports.requireMatchesAccess = requireMatchesAccess;
        ...
        try {
            ...
            switch (resourceType) {
                ...
                case 'chat':
                    ...
                    const chat = await Chat_1.ChatModel.findById(resourceId).populate('matchId');
                    if (chat) {
                        const match = await Match_1.MatchModel.findById(chat.matchId);
                        if (match) {
                            isOwner = match.userA.toString() === userId || match.userB.toString() === userId;
                        }
                    }
                    break;
                case 'match':
                    // Check if user is part of the match
                    const match = await Match_1.MatchModel.findById(resourceId);
                    if (match) {
                        isOwner = match.userA.toString() === userId || match.userB.toString() === userId;
                    }
                    break;
 ...
 */
function requireSwipePermission() {
    return async (req, res, next) => {
        try {
            const userId = req.user?.id;
            ...
            const existingSwipe = await Match_1.MatchModel.findOne({
                userA: userId,
                userB: targetUserId
            });
            if (existingSwipe) {
